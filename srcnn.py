...
class SRCNN(nn.Module):
    def __init__(self):
        ...
        # To make sure the HR-Base and the HR-SRCNN have the same size,
        # the formula n(out)={[n(in)+2p-k]/s}+1 is used to calculate the value of padding.
...
print(output_[0, 0].shape)
SR_image = output_[0,0].numpy()

# print the input
print('HR-Base', LR_image.shape)
iio.imsave('HR-Base.bmp', LR_image)

# print SRCNN result
print('HR-SRCNN', SR_image.shape)
iio.imsave('HR-SRCNN.bmp', SR_image)

# print ground truth of this case
print('GT', HR_image.shape)
iio.imsave('GT.bmp', HR_image)

print('PSNR between GT and HR-SRCNN: ', skimage.metrics.peak_signal_noise_ratio(HR_image, SR_image))
print('PSNR between GT and HR-Base: ', skimage.metrics.peak_signal_noise_ratio(HR_image, LR_image))
